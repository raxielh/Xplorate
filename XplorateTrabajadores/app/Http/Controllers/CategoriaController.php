...
class CategoriaController extends AppBaseController
{
    ...
    public function show($id)
    {
        ...
        $preguntas = DB::select("
            SELECT p.`id`,p.`descripcion`,nombre,p.`orden`,tp.descripcion as tp,sn.descripcion as sn FROM `preguntas` p,`estados` e,tipo_pregunta tp,siono sn WHERE p.`estado`=e.`id` AND p.tipo_pregunta=tp.id AND p.dependiente=sn.id AND categoria=".$id."  ORDER BY orden ASC
        ");
        if (empty($categoria)) {
        ...
        return view('categorias.show')->with('categoria', $categoria)->with('preguntas', $preguntas)->with('tipo_pregunta', $tipo_pregunta)->with('siono', $siono)->with('preguntadependiente', $preguntadependiente);
    }
